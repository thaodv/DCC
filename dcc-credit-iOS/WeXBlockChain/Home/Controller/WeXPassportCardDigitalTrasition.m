//
//  WeXPassportCardDigitalTrasition.m
//  WeXBlockChain
//
...
@interface WeXPassportCardDigitalTrasition()
...
@implementation WeXPassportCardDigitalTrasition
...
- (void)doPushAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    WeXWalletDigitalAssetListController *toVC = (WeXWalletDigitalAssetListController *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    ...
    //    UIView *tempView = [toVC.cardView snapshotViewAfterScreenUpdates:NO];
    ...
    [UIView animateWithDuration:[self transitionDuration:transitionContext] delay:0.0 usingSpringWithDamping:0.55 initialSpringVelocity:1 / 0.55 options:0 animations:^{
        ...
        //        tempView.frame = [toVC.cardView convertRect:toVC.cardView.bounds toView:containerView];
        CGRect rect = [toVC.cardView convertRect:cell.cardView.bounds toView:containerView];
        ...
        //        UIView *superView = toVC.cardView.superview;
        //        [toVC.cardView mas_updateConstraints:^(MASConstraintMaker *make) {
        //            make.top.equalTo(superView).offset(kNavgationBarHeight);
        //            make.leading.equalTo(superView).offset(0);
        //            make.trailing.equalTo(superView).offset(0);
        //            make.bottom.equalTo(superView);
        //        }];
        //如果动画过渡取消了就标记不完成，否则才完成，这里可以直接写YES，如果有手势过渡才需要判断，必须标记，否则系统不会中动画完成的部署，会出现无法交互之类的bug
        [transitionContext completeTransition:YES];
        
        toVC.tempView = tempView;
...
//        [toVC.view layoutIfNeeded];
////        tempView.frame = [toVC.cardView convertRect:toVC.cardView.bounds toView:containerView];
//        tempView.frame = [toVC.cardView convertRect:cell.cardView.bounds toView:containerView];
//        NSLog(@"tempView.frame=%@",NSStringFromCGRect(tempView.frame));
//        toVC.view.alpha = 1;
//    } completion:^(BOOL finished) {
//        tempView.hidden = YES;
...
- (void)doPopAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    WeXWalletDigitalAssetListController *fromVC = (WeXWalletDigitalAssetListController *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
...
//    UIView *tempView = containerView.subviews.lastObject;
    ...
    UIView *tempView = fromVC.tempView;
    //设置初始状态
    cell.cardView.hidden = YES;
