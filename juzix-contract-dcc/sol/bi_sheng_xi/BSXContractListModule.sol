...
contract BSXContractListModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function BSXContractListModule() {
	  //定义模块合约名称
        string memory moduleName = "BSXContractListModule";

        //定义模块合约名称
        string memory moduleDesc = "BSXContractListModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

	//把合约注册到JUICE链上
        LibLog.log("register BSXContractListModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add BSXContractListModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add BSXContractListModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
        //tmpModule.moduleName = "BSXContractListModule";//设置模块名称【需要修改】
