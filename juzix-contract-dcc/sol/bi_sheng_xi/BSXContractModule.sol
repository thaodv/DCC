...
contract BSXContractModule is BaseModule {
    using LibModule for *;
	...
	//把合约注册到JUICE链上
        LibLog.log("register BSXContractModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add BSXContractModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add BSXContractModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
