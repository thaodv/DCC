...
contract UseModule is BaseModule {
    using LibModule for *;
    ...
    function UseModule() {
	  //定义模块合约名称
        string memory moduleName = "UseModule";

        //定义模块合约名称
        string memory moduleDesc = "UseModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

	//把合约注册到JUICE链上
        LibLog.log("register UseModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add UseModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add UseModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
        //tmpModule.moduleName = "UseModule";//设置模块名称【需要修改】
