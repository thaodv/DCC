...
contract TNLoanFeeServiceModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function TNLoanFeeServiceModule() {
        //定义模块合约名称
        string memory moduleName = "TNLoanFeeServiceModule";

        //定义模块合约名称
        string memory moduleDesc = "TNLoanFeeServiceModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

        //把合约注册到JUICE链上
        LibLog.log("register TNLoanFeeServiceModule");
        ...
        LibLog.log("add TNLoanFeeServiceModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add TNLoanFeeServiceModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
