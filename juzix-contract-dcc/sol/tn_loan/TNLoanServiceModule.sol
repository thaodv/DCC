...
contract TNLoanServiceModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function TNLoanServiceModule() {
        //定义模块合约名称
        string memory moduleName = "TNLoanServiceModule";

        //定义模块合约名称
        string memory moduleDesc = "TNLoanServiceModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

        //把合约注册到JUICE链上
        LibLog.log("register TNLoanServiceModule");
        ...
        LibLog.log("add TNLoanServiceModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add TNLoanServiceModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
