...
contract IpfsTokenDigest3 is OwnerPermission{
    ...
    event ipfsTokenPut(address indexed userAddress, address indexed contractAddress, uint256 version,string cipher,string token,bytes iv,bytes digest1,bytes digest2,bytes digest3,uint256 keyHashVersion);

    function IpfsTokenDigest3(address ipfsKeyHashAddress) public{
        register("IpfsTokenDigest3Module", "0.0.1.0", "IpfsTokenDigest3", "0.0.1.0");
    ...
    function putIpfsToken(address contractAddress,uint256 version,string cipher,string token,bytes iv,bytes digest1,bytes digest2,bytes digest3,bytes32 keyHash)  public {
        ...
        if(!(digest3.length <= IPFS_DIGEST_MAXSIZE)){
            log("!(digest3.length <= IPFS_DIGEST_MAXSIZE)");
            throw;
        }
        if(!(keyHash!=0)){
            log("!(keyHash!=0)");
        ...
        ipfsCertData[msg.sender][contractAddress] = IpfsTokenData(version,cipher,token,iv,digest1,digest2,digest3,v1);
        ipfsTokenPut(msg.sender, contractAddress,version,cipher,token,iv,digest1,digest2,digest3,v1);
    }
