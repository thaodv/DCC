...
contract AgreementService2Module is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function AgreementService2Module() {
        //定义模块合约名称
        string memory moduleName = "AgreementService2Module";

        //定义模块合约名称
        string memory moduleDesc = "AgreementService2Module";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

        //把合约注册到JUICE链上
        LibLog.log("register AgreementService2Module");
        ...
        LibLog.log("add AgreementService2Module to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add AgreementService2Module to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
