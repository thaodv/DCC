...
contract CertServiceModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function CertServiceModule() {
	  //定义模块合约名称
        string memory moduleName = "CertServiceModule";
	...
	//把合约注册到JUICE链上
        LibLog.log("register CertServiceModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add CertServiceModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add CertServiceModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
