...
contract CertService4ImplModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function CertService4ImplModule() {
	  //定义模块合约名称
        string memory moduleName = "CertService4ImplModule";

        //定义模块合约名称
        string memory moduleDesc = "CertService4ImplModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

	//把合约注册到JUICE链上
        LibLog.log("register CertService4ImplModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add CertService4ImplModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add CertService4ImplModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
        //tmpModule.moduleName = "CertService4ImplModule";//设置模块名称【需要修改】
