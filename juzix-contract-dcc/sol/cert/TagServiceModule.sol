...
contract TagServiceModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function TagServiceModule() {
	  //定义模块合约名称
        string memory moduleName = "TagServiceModule";
	...
	//把合约注册到JUICE链上
        LibLog.log("register TagServiceModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add TagServiceModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add TagServiceModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
        //tmpModule.moduleName = "TagServiceModule";//设置模块名称【需要修改】
