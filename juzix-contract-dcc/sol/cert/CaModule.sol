...
contract CaModule is BaseModule {
    using LibModule for *;
    ...
    function CaModule() {
        //定义模块合约名称
        string memory moduleName = "CaModule";

        //定义模块合约名称
        string memory moduleDesc = "ca模块";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

        //把合约注册到JUICE链上
        LibLog.log("register CaModule");
        ...
        LibLog.log("add CaModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add CaModule to SysModule failed");
            return;
        }
    }
