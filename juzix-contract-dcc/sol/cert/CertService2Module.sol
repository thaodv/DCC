...
contract CertService2Module is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function CertService2Module() {
	  //定义模块合约名称
        string memory moduleName = "CertService2Module";

        //定义模块合约名称
        string memory moduleDesc = "CertService2Module";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

	//把合约注册到JUICE链上
        LibLog.log("register CertService2Module");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add CertService2Module to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add CertService2Module to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
        //tmpModule.moduleName = "CertService2Module";//设置模块名称【需要修改】
