...
contract IdCertServiceModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function IdCertServiceModule() {
	  //定义模块合约名称
        string memory moduleName = "IdCertServiceModule";
	...
	//把合约注册到JUICE链上
        LibLog.log("register IdCertServiceModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add IdCertServiceModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add IdCertServiceModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
        //tmpModule.moduleName = "IdCertServiceModule";//设置模块名称【需要修改】
