...
contract UserPropertyServiceModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function UserPropertyServiceModule() {
        //定义模块合约名称
        string memory moduleName = "UserPropertyServiceModule";

        //定义模块合约名称
        string memory moduleDesc = "UserPropertyServiceModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

        //把合约注册到JUICE链上
        LibLog.log("register UserPropertyServiceModule");
        ...
        LibLog.log("add UserPropertyServiceModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add UserPropertyServiceModule to SysModule failed");
            return;
        }
        //添加模块
        //tmpModule.clear();
