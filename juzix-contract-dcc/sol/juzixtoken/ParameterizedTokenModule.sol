...
contract ParameterizedTokenModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function ParameterizedTokenModule() {
	  //定义模块合约名称
        string memory moduleName = "ParameterizedTokenModule";

        //定义模块合约名称
        string memory moduleDesc = "ParameterizedTokenModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

	//把合约注册到JUICE链上
        LibLog.log("register ParameterizedTokenModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add ParameterizedTokenModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add ParameterizedTokenModule to SysModule failed");
            return;
        }
    }
