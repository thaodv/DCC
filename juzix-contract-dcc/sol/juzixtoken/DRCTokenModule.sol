...
contract DRCTokenModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function DRCTokenModule() {
	  //定义模块合约名称
        string memory moduleName = "DRCTokenModule";

        //定义模块合约名称
        string memory moduleDesc = "DRCTokenModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

	//把合约注册到JUICE链上
        LibLog.log("register DRCTokenModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add DRCTokenModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add DRCTokenModule to SysModule failed");
            return;
        }
    }
