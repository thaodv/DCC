...
contract RNSTokenModule is BaseModule {
    using LibModule for *;
    ...
    //模块构造函数
    function RNSTokenModule() {
	  //定义模块合约名称
        string memory moduleName = "RNSTokenModule";

        //定义模块合约名称
        string memory moduleDesc = "RNSTokenModule";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

	//把合约注册到JUICE链上
        LibLog.log("register RNSTokenModule");
	...
	tmpModule.moduleName = moduleName;
        ...
        LibLog.log("add RNSTokenModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add RNSTokenModule to SysModule failed");
            return;
        }
    }
