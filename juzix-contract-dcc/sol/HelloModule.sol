...
contract HelloModule is BaseModule {
    using LibModule for *;
    ...
    function HelloModule() {
        //定义模块合约名称
        string memory moduleName = "HelloModule";

        //定义模块合约名称
        string memory moduleDesc = "hello模块";

        //定义模块合约版本号
        string memory moduleVersion = "0.0.1.0";

        //把合约注册到JUICE链上
        LibLog.log("register HelloModule");
        ...
        LibLog.log("add HelloModule to SysModule");
        uint ret = addModule(tmpModule.toJson());

        if (ret != 0) {
            LibLog.log("add HelloModule to SysModule failed");
            return;
        }
    }
