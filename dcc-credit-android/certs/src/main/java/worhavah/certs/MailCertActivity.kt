...
class MailCertActivity : BindActivity<ActivityMailcertBinding>(){
    override val contentLayoutId: Int
        get() = R.layout.activity_mailcert
    var emailAddress=""
    ...
    private fun submitVerify(passport: Passport, address: String) {
        CertOperations.submitCert(passport, address,  { this.orderId = it })
            .observeOn(AndroidSchedulers.mainThread())
            .doOnSubscribe {
                showLoadingDialog()
            }
            ...
            .subscribe({order->
                ScfOperations
                    .withScfTokenInCurrentPassport {
                        CertOperations.insCertApi.mailverif(it,this.orderId,binding.etPn.text.toString())
                    }.observeOn(AndroidSchedulers.mainThread())
                    .subscribe({
                        toast("验证码已发送")
                    }, { it.printStackTrace()})
