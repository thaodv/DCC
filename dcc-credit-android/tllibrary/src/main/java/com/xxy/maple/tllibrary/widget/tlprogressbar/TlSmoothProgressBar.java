 ...
 */
public class TlSmoothProgressBar extends ProgressBar {
  ...
  public TlSmoothProgressBar(Context context) {
    this(context, null);
  }

  public TlSmoothProgressBar(Context context, AttributeSet attrs) {
    this(context, attrs, R.attr.spbStyle);
  }

  public TlSmoothProgressBar(Context context, AttributeSet attrs, int defStyle) {
    super(context, attrs, defStyle);

    if (isInEditMode()) {
      setIndeterminateDrawable(new TlSmoothProgressDrawable.Builder(context).build());
      return;
    }
    ...
    TlSmoothProgressDrawable.Builder builder = new TlSmoothProgressDrawable.Builder(context)
        .speed(speed)
    ...
    TlSmoothProgressDrawable d = builder.build();
    ...
    if (a.hasValue(R.styleable.SmoothProgressBar_spb_generate_background_with_colors)) {
      if (a.getBoolean(R.styleable.SmoothProgressBar_spb_generate_background_with_colors, false)) {
        setSmoothProgressDrawableBackgroundDrawable(
            TlSmoothProgressBarUtils.generateDrawableWithColors(checkIndeterminateDrawable().getColors(), checkIndeterminateDrawable().getStrokeWidth()));
      }
    }
  ...
  protected synchronized void onDraw(Canvas canvas) {
    super.onDraw(canvas);
    if (isIndeterminate() && getIndeterminateDrawable() instanceof TlSmoothProgressDrawable &&
        !((TlSmoothProgressDrawable) getIndeterminateDrawable()).isRunning()) {
      getIndeterminateDrawable().draw(canvas);
    }
  }

  private TlSmoothProgressDrawable checkIndeterminateDrawable() {
    Drawable ret = getIndeterminateDrawable();
    if (ret == null || !(ret instanceof TlSmoothProgressDrawable))
      throw new RuntimeException("The drawable is not a SmoothProgressDrawable");
    return (TlSmoothProgressDrawable) ret;
  }
  ...
  public void setInterpolator(Interpolator interpolator) {
    ...
    Drawable ret = getIndeterminateDrawable();
    if (ret != null && (ret instanceof TlSmoothProgressDrawable))
      ((TlSmoothProgressDrawable) ret).setInterpolator(interpolator);
  }
  ...
  public void setSmoothProgressDrawableCallbacks(TlSmoothProgressDrawable.Callbacks listener) {
