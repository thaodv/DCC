...
@Database(entities = [CaAuthRecord::class, AuthKeyChangeRecord::class, CurrencyMeta::class, BeneficiaryAddress::class, QueryHistory::class, TransRecord::class],
    ...
    companion object {
        ...
        private val migration_2_3 = object : Migration(VERSION_2, VERSION_3) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                database.execSQL("ALTER TABLE `${BeneficiaryAddress.TABLE_NAME}` ADD COLUMN '${BeneficiaryAddress.COLUMN_AVATAR}' TEXT")
                database.execSQL("ALTER TABLE `${BeneficiaryAddress.TABLE_NAME}` ADD COLUMN '${BeneficiaryAddress.COLUMN_CREATE_TIME}' LONG")
                database.execSQL("ALTER TABLE `${BeneficiaryAddress.TABLE_NAME}` ADD COLUMN '${BeneficiaryAddress.COLUMN_UPDATE_TIME}' LONG")
                database.execSQL("CREATE TABLE IF NOT EXISTS `${TransRecord.TABLE_NAME}` (`id` INTEGER NOT NULL,`address` TEXT NOT NULL, `short_name` TEXT, `avatar_url` TEXT,`is_add` INTEGER,`create_time` LONG,`update_time` LONG, PRIMARY KEY(`id`))")
            }
        }
