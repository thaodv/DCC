...
import io.wexchain.dcc.databinding.DialogConfirmPublic2privateBinding
import io.wexchain.digitalwallet.EthsTransactionScratch

/**
 * Created by sisel on 2018/1/22.
 */
class Public2PrivateConfirmDialogFragment : DialogFragment() {
    var isEdit = false

    private lateinit var binding: DialogConfirmPublic2privateBinding

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = DataBindingUtil.inflate(inflater, R.layout.dialog_confirm_public2private, container, false)
        val scratch = getScratch()
        val app = App.get()
        val passport = app.passportRepository.getCurrentPassport()!!
        val vm = Public2PrivateConfirmVm(scratch, passport, app.assetsRepository, isEdit)
        vm.syncProtect(this)
    ...
    companion object {
        ...
        fun create(ethsTransactionScratch: EthsTransactionScratch, isEdit: Boolean = false): Public2PrivateConfirmDialogFragment {
            return Public2PrivateConfirmDialogFragment().apply {
