 ...
 * @author zhengpeng
 */
public class LoanOrderAuditPatroller {

	private static final Logger logger = LoggerFactory.getLogger(LoanOrderAuditPatroller.class);
	...
	public void patrol() {
		...
		lockTemplate.execute("LoanOrderAuditPatroller", null, () -> {
			logger.debug("Loan order audit patrol start");
			...
			int count = loanOrderRepository.countByCreatedTimeBetweenAndStatusIn(
					beginTime, endTime,
					Collections.singletonList(LoanOrderStatus.AUDITING));
			...
			logger.debug("Loan order audit patrol end");
			return null;
		});
	}
