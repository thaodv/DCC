...
contract TNLoanService is OperatorPermission, GateControl, FastFailure {
    ...
    function TNLoanService(address agreementContractAddress) public {
    ...
    function deliverFailure(uint256 id) onlyOperator external {
        ...
        Status[] memory exptectedStatusArray = new  Status[](1);
        exptectedStatusArray[0] = Status.AUDITING;
        checkAndChangeStatus(order, exptectedStatusArray, Status.FAILURE);
        if (loanFee != address(0)) {
            loanFee.send(order.borrower,order.fee);
        }
    }
